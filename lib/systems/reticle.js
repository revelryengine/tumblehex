...
import { ReticleModel } from './models/reticle.js';
...
export class ReticleSystem extends System {
    ...
    update() {
        ...
        this.reticle.transform.changed = true;

        if(!this.action && (keys.Space || pointers.primary?.click)) {
            this.action = true;
            for(const tile of this.guideTiles) {
                this.stage.spawn(TileModel, { transform: {}, hexcoords: { q: tile.hex.q, r: tile.hex.r }, color: tile.color });
    ...
    gameover(win) {
        ...
        for(let i = 0; i < reticle.guides.length; i++) {
            reticle.transform.scale = [0, 0, 0];
            reticle.transform.changed = true;
        }
